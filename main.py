            ...
            if stream != None:
                if path.isfile(channel):
                    ...
                    if send_pushover_notifications:
                        pythonNotify.sendPushoverNotification(pushover_app_key, pushover_user_key, 'There is a new stream recording now for the channel: %s!' % (channel), channel + ' now recording!', 0)
